...
describe("Recipe API Integration Tests", () => {
    ...
    beforeAll(async () => {
        ...
        await (0, supertest_1.default)(index_1.app).post("/api/v1/auth/register").send({
            name: "admin user",
            email: "admin@example.com",
            password: "password",
            role: "admin",
        });
        const adminRes = await (0, supertest_1.default)(index_1.app).post("/api/v1/auth/login").send({
            email: "admin@example.com",
            password: "password",
        });
        ...
        const editorRes = await (0, supertest_1.default)(index_1.app).post("/api/v1/auth/login").send({
            email: "editor@example.com",
            password: "password",
        });
        ...
        const userRes = await (0, supertest_1.default)(index_1.app).post("/api/v1/auth/login").send({
            email: "user@example.com",
            password: "password",
        });
    ...
    test("list should return only owner recipes", async () => {
        const adminList = await (0, supertest_1.default)(index_1.app)
            .get("/api/v1/recipes")
            .set("Authorization", `Bearer ${adminToken}`)
            .expect(200);
        const editorList = await (0, supertest_1.default)(index_1.app)
            .get("/api/v1/recipes")
            .set("Authorization", `Bearer ${editorToken}`)
            .expect(200);
        const adminIds = adminList.body.recipes.map((r) => String(r._id));
        const editorIds = editorList.body.recipes.map((r) => String(r._id));
        ...
        const userList = await (0, supertest_1.default)(index_1.app)
            .get("/api/v1/recipes")
            .set("Authorization", `Bearer ${userToken}`)
            .expect(200);
        const userIds = userList.body.recipes.map((r) => String(r._id));
    ...
    test("favorites should be visible only for current user", async () => {
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            .expect(200);
        const adminFavorites = await (0, supertest_1.default)(index_1.app)
            .get("/api/v1/users/me/favorites")
            .set("Authorization", `Bearer ${adminToken}`)
            .expect(200);
        const editorFavorites = await (0, supertest_1.default)(index_1.app)
            .get("/api/v1/users/me/favorites")
            .set("Authorization", `Bearer ${editorToken}`)
            .expect(200);
        const adminFavoriteIds = adminFavorites.body.items.map((f) => f.id);
        const editorFavoriteIds = editorFavorites.body.items.map((f) => f.id);
    ...
    test("user can add only external favorites", async () => {
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            .expect(200);
        const userFavorites = await (0, supertest_1.default)(index_1.app)
            .get("/api/v1/users/me/favorites")
            .set("Authorization", `Bearer ${userToken}`)
            .expect(200);
        expect(userFavorites.body.items.some((item) => item.id === "ext-123" && item.source === "external")).toBe(true);
        expect(userFavorites.body.items.some((item) => item.source === "internal")).toBe(false);
    });
    // ---- note tests ----
